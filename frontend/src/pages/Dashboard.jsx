...
export default function Dashboard() {
  ...
  const [folderSubTab, setFolderSubTab] = useState('certificates');
  ...
  // Query: payment history for selected folder user
  const { data: userPaymentsData, isLoading: loadingUserPayments, refetch: refetchUserPayments } = useQuery({
    queryKey: ['adminUserPayments', selectedFolderUser?._id],
    queryFn: async () => {
      if (!selectedFolderUser?._id) return [];
      const res = await api.get(`/api/admin/users/${selectedFolderUser._id}/payment-history`);
      return res.data.payments || [];
    },
    enabled: !!selectedFolderUser?._id,
  });

  const toggleGovIdMutation = useMutation({
    mutationFn: async ({ userId, isGovIdVerified }) => {
      const res = await api.put(`/api/admin/users/${userId}/verify-gov-id`, { isGovIdVerified });
      return res.data;
    },
    onSuccess: (data) => {
      toast.success(data.message || 'ID verification status updated!');
      queryClient.invalidateQueries({ queryKey: ['adminUsersMonitor'] });
    },
    onError: (err) => {
        ...
        <div className="space-y-6">
          {!selectedFolderUser ? (
            ...
            <div className="space-y-6">
              ...
              {/* Folder Sub-Tabs */}
              <div className="flex border-b border-purple-950/20 mb-4 gap-4">
                <button
                  onClick={() => setFolderSubTab('certificates')}
                  className={`pb-2.5 text-xs font-bold uppercase tracking-wider border-b-2 transition-all ${
                    folderSubTab === 'certificates'
                      ? 'border-accent text-white font-extrabold'
                      : 'border-transparent text-gray-500 hover:text-gray-300'
                  }`}
                >
                  📄 Certificates ({activeFolderUser?.certificates?.length || 0})
                </button>
                <button
                  onClick={() => setFolderSubTab('verification')}
                  className={`pb-2.5 text-xs font-bold uppercase tracking-wider border-b-2 transition-all flex items-center gap-1.5 ${
                    folderSubTab === 'verification'
                      ? 'border-accent text-white font-extrabold'
                      : 'border-transparent text-gray-500 hover:text-gray-300'
                  }`}
                >
                  💳 Gov. ID & Payment History
                </button>
              </div>
              ...
              {folderSubTab === 'certificates' ? (
                ...
                // Gov. ID & Payment History Tab Content
                <div className="space-y-6">
                  {/* Gov. ID verification control card */}
                  <div className="glass-panel p-6 rounded-2xl border-purple-950/40 bg-[#0c0a13]/70 space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <h4 className="font-accent text-sm font-bold text-white uppercase tracking-wider">Government ID Verification</h4>
                        <p className="text-xs text-gray-400 mt-1">Verify Government ID credentials to unlock the green checkmark badge on user's profile.</p>
                      </div>
                      <div className="flex items-center gap-2">
                        {selectedFolderUser.isGovIdVerified ? (
                          <span className="bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-[10px] font-bold px-3 py-1 rounded-full flex items-center gap-1">
                            <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" /> ID Verified
                          </span>
                        ) : (
                          <span className="bg-red-500/15 border border-red-500/30 text-red-400 text-[10px] font-bold px-3 py-1 rounded-full">
                            Unverified
                          </span>
                        )}
                      </div>
                    ...
                    <div className="pt-2 border-t border-purple-950/20 flex gap-3">
                      <button
                        onClick={() => {
                          const nextVerified = !selectedFolderUser.isGovIdVerified;
                          // update local selected user copy
                          setSelectedFolderUser({ ...selectedFolderUser, isGovIdVerified: nextVerified });
                          toggleGovIdMutation.mutate({ userId: selectedFolderUser._id, isGovIdVerified: nextVerified });
                        }}
                        disabled={toggleGovIdMutation.isLoading}
                        className={`text-xs font-bold px-4 py-2 rounded-xl transition-all ${
                          selectedFolderUser.isGovIdVerified
                            ? 'bg-red-650/25 border border-red-500/30 text-red-400 hover:bg-red-650/40'
                            : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-500/10'
                        }`}
                      >
                        {toggleGovIdMutation.isLoading ? 'Processing...' : selectedFolderUser.isGovIdVerified ? 'Revoke Verification' : 'Approve Government ID'}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="glass-panel p-6 rounded-2xl border-purple-950/40 bg-[#0c0a13]/70 space-y-4">
                    <h4 className="font-accent text-sm font-bold text-white uppercase tracking-wider">Premium Payment History</h4>
                    ...
                    {loadingUserPayments ? (
                      ...
                      <p className="text-xs text-gray-500 py-4 text-center">No payment transactions recorded for this user.</p>
                    ) : (
                      <div className="overflow-x-auto rounded-xl border border-purple-950/30 bg-[#07050d]">
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="bg-purple-950/20 border-b border-purple-950/40 text-gray-400 uppercase text-[9px] font-bold tracking-widest">
                          ...
                          <tbody className="divide-y divide-purple-950/20 text-xs text-gray-300">
                            {userPaymentsData.map((payment) => (
                              <tr key={payment._id} className="hover:bg-[#12111d]/20 transition-colors">
                                <td className="px-4 py-3 font-semibold text-white">{payment.planName}</td>
                                <td className="px-4 py-3 text-purple-300">₹{payment.amount}</td>
                                <td className="px-4 py-3 font-mono text-gray-400">{payment.utrNumber}</td>
